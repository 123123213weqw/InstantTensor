...
For local and local/<git-ref>, the backend is siphon and the package is
installed from a temporary clean copy of this repository after checking out the ref.
For siphon backend, only local specs are supported.
"""
...
# Add custom indexes or constraints here, or set SIPHON_BENCH_PIP_ARGS.
...
def parse_backend_spec(spec: str) -> dict[str, str | None]:
    ...
    if backend == "siphon":
        raise BenchError(
            "Siphon backend no longer supports pypi installs. Use local/<git-ref> instead."
        )
...
def siphon_requirement(version: str | None) -> str:
    return package_requirement("siphon[test]", version)
...
def pip_install(
    ...
    *,
    ...
    extra_args = EXTRA_PIP_ARGS + shlex.split(
        os.environ.get("SIPHON_BENCH_PIP_ARGS", "")
    )
...
def run_case(
    *,
    ...
    if backend == "siphon":
        test_env.setdefault("SIPHON_DEBUG", "1")
...
def stable_work_dir() -> Path:
    return Path(tempfile.gettempdir()) / "siphon-bench"
