...
# import zlib
import siphon
from siphon import safe_open as instant_safe_open
...
def parse_args():
    ...
    parser.add_argument(
        'backend',
        type=str,
        choices=['siphon', 'safetensors', 'runai_model_streamer', 'fastsafetensors'],
        help='Backend type (choices: siphon, safetensors, runai_model_streamer, fastsafetensors)'
    )
...
grouped = True if backend == 'siphon' else False
...
def siphon_iterator(files, device, process_group):
    global t_open, t_close, t_keys, t_first
    t_open_record = time.perf_counter()
